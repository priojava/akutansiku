@extends('layouts.app')

@section('title', 'Laporan Neraca Saldo (Trial Balance)')

@push('styles')
<style>
    @media print {
        @page {
            size: A4 portrait;
            margin: 12mm 10mm 12mm 10mm;
        }
        * {
            -webkit-print-color-adjust: exact !important;
            print-color-adjust: exact !important;
        }
        body {
            background: #ffffff !important;
            color: #000000 !important;
            font-size: 8.5pt !important;
        }
        aside, nav, header, .no-print, button, form {
            display: none !important;
        }
        .main-content, main, .space-y-4 {
            margin: 0 !important;
            padding: 0 !important;
            width: 100% !important;
        }
        .print-header {
            display: block !important;
            margin-bottom: 12px !important;
            border-bottom: 2px solid #0f172a !important;
            padding-bottom: 6px !important;
        }
        .kpi-cards {
            display: none !important;
        }
        .card-table {
            border: 1px solid #cbd5e1 !important;
            box-shadow: none !important;
            page-break-inside: auto !important;
        }
        tr {
            page-break-inside: avoid !important;
        }
    }
</style>
@endpush

@section('content')
@php
    $getGroupKey = function($row) {
        $code = (string)($row['code'] ?? '');
        $cat = (string)($row['category'] ?? '');

        if (str_starts_with($code, '1-107') || str_starts_with($code, '1-108') || str_starts_with($code, '1-109') || in_array($cat, ['Aset Tetap', 'Akumulasi Penyusutan', 'Aset Lainnya'])) {
            return 'aset_tidak_lancar';
        }
        if (str_starts_with($code, '1-') || in_array($cat, ['Kas & Bank', 'Akun Piutang', 'Persediaan', 'Aset Lancar Lainnya'])) {
            return 'aset_lancar';
        }
        if (str_starts_with($code, '2-') || in_array($cat, ['Akun Hutang', 'Kewajiban Lancar Lainnya', 'Kewajiban Jangka Panjang'])) {
            return 'kewajiban';
        }
        if (str_starts_with($code, '3-') || in_array($cat, ['Ekuitas'])) {
            return 'ekuitas';
        }
        if (str_starts_with($code, '4-') || in_array($cat, ['Pendapatan'])) {
            return 'pendapatan';
        }
        if (str_starts_with($code, '5-') || in_array($cat, ['Harga Pokok Penjualan'])) {
            return 'hpp';
        }
        if (str_starts_with($code, '6-') || str_starts_with($code, '7-') || in_array($cat, ['Beban', 'Beban Operasional'])) {
            return 'beban';
        }
        return 'lainnya';
    };

    $groupMeta = [
        'aset_lancar' => [
            'name' => 'ASET LANCAR (CURRENT ASSETS)',
            'icon' => 'fa-solid fa-coins',
            'bg' => 'bg-gradient-to-r from-blue-700 to-cyan-700',
            'border' => 'border-blue-200',
            'subtotal_bg' => 'bg-blue-50/70',
        ],
        'aset_tidak_lancar' => [
            'name' => 'ASET TETAP & TIDAK LANCAR (FIXED ASSETS)',
            'icon' => 'fa-solid fa-building-columns',
            'bg' => 'bg-gradient-to-r from-indigo-700 to-blue-800',
            'border' => 'border-indigo-200',
            'subtotal_bg' => 'bg-indigo-50/70',
        ],
        'kewajiban' => [
            'name' => 'KEWAJIBAN / LIABILITAS (LIABILITIES)',
            'icon' => 'fa-solid fa-file-invoice-dollar',
            'bg' => 'bg-gradient-to-r from-amber-600 to-orange-700',
            'border' => 'border-amber-200',
            'subtotal_bg' => 'bg-amber-50/70',
        ],
        'ekuitas' => [
            'name' => 'EKUITAS & MODAL (EQUITY)',
            'icon' => 'fa-solid fa-scale-balanced',
            'bg' => 'bg-gradient-to-r from-purple-700 to-violet-800',
            'border' => 'border-purple-200',
            'subtotal_bg' => 'bg-purple-50/70',
        ],
        'pendapatan' => [
            'name' => 'PENDAPATAN USAHA (REVENUE)',
            'icon' => 'fa-solid fa-arrow-trend-up',
            'bg' => 'bg-gradient-to-r from-emerald-600 to-teal-700',
            'border' => 'border-emerald-200',
            'subtotal_bg' => 'bg-emerald-50/70',
        ],
        'hpp' => [
            'name' => 'HARGA POKOK PENJUALAN (COGS)',
            'icon' => 'fa-solid fa-boxes-stacked',
            'bg' => 'bg-gradient-to-r from-slate-700 to-zinc-800',
            'border' => 'border-slate-200',
            'subtotal_bg' => 'bg-slate-100/80',
        ],
        'beban' => [
            'name' => 'BEBAN OPERASIONAL (OPERATING EXPENSES)',
            'icon' => 'fa-solid fa-chart-pie',
            'bg' => 'bg-gradient-to-r from-rose-600 to-red-700',
            'border' => 'border-rose-200',
            'subtotal_bg' => 'bg-rose-50/70',
        ],
        'lainnya' => [
            'name' => 'PENDAPATAN & BEBAN NON-OPERASIONAL',
            'icon' => 'fa-solid fa-arrows-split-up-and-left',
            'bg' => 'bg-gradient-to-r from-slate-600 to-gray-700',
            'border' => 'border-slate-200',
            'subtotal_bg' => 'bg-slate-100/80',
        ],
    ];

    $groupedRows = [];
    foreach ($report['rows'] as $row) {
        $key = $getGroupKey($row);
        $groupedRows[$key][] = $row;
    }

    $isBalanced = abs($report['total_debit'] - $report['total_credit']) < 0.01;
    $balanceDiff = abs($report['total_debit'] - $report['total_credit']);
@endphp

<div class="space-y-4">

    <!-- Print Header (Hanya tampil saat Print/PDF) -->
    <div class="hidden print-header">
        <div class="flex justify-between items-end">
            <div>
                <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">{{ $company->name ?? 'PERUSAHAAN' }}</h1>
                <h2 class="text-sm font-semibold text-blue-800">LAPORAN NERACA SALDO (TRIAL BALANCE)</h2>
                <p class="text-[11px] text-slate-500">Periode: {{ \Carbon\Carbon::parse($startDate)->isoFormat('D MMMM Y') }} s/d {{ \Carbon\Carbon::parse($endDate)->isoFormat('D MMMM Y') }}</p>
            </div>
            <div class="text-right text-[10px] text-slate-400">
                <p>Status: {{ $isBalanced ? 'SEIMBANG' : 'SELISIH' }}</p>
                <p>Dicetak pada: {{ \Carbon\Carbon::now()->isoFormat('D MMMM Y HH:mm') }}</p>
            </div>
        </div>
    </div>

    <!-- Header Filter & Action (Kompak) -->
    <div class="no-print bg-white rounded-xl border border-slate-200 p-3 shadow-xs flex flex-wrap items-center justify-between gap-3">
        <form method="GET" action="{{ route('reports.trial_balance') }}" class="flex flex-wrap items-center gap-2">
            <div class="flex items-center bg-slate-50 border border-slate-300 rounded-lg px-2.5 py-1 text-xs text-slate-700">
                <i class="fa-regular fa-calendar text-slate-400 mr-2 text-[11px]"></i>
                <input type="date" name="start_date" value="{{ $startDate }}" class="bg-transparent focus:outline-none font-medium text-xs">
                <span class="mx-1.5 text-slate-400">&rarr;</span>
                <input type="date" name="end_date" value="{{ $endDate }}" class="bg-transparent focus:outline-none font-medium text-xs">
            </div>

            <button type="submit" class="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg shadow-xs transition flex items-center space-x-1 cursor-pointer">
                <i class="fa-solid fa-filter text-[10px]"></i>
                <span>Filter</span>
            </button>
        </form>

        <div class="flex items-center space-x-2">
            <button type="button" onclick="exportTrialBalanceToExcel()" class="px-3 py-1 bg-white border border-slate-300 hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-700 text-slate-700 text-xs font-semibold rounded-lg shadow-xs transition flex items-center space-x-1.5 cursor-pointer">
                <i class="fa-regular fa-file-excel text-emerald-600 text-xs"></i>
                <span>Excel</span>
            </button>
            <button type="button" onclick="window.print()" class="px-3 py-1 bg-white border border-slate-300 hover:bg-rose-50 hover:border-rose-300 hover:text-rose-700 text-slate-700 text-xs font-semibold rounded-lg shadow-xs transition flex items-center space-x-1.5 cursor-pointer">
                <i class="fa-regular fa-file-pdf text-rose-600 text-xs"></i>
                <span>Cetak / PDF</span>
            </button>
        </div>
    </div>

    <!-- KPI Summary Cards (Kompak & Elegan) -->
    <div class="kpi-cards grid grid-cols-2 lg:grid-cols-4 gap-3">
        <!-- Total Debit -->
        <div class="bg-white rounded-xl border border-slate-200/90 p-2.5 sm:p-3 shadow-xs flex items-center justify-between">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Debit</p>
                <h3 class="text-sm sm:text-base font-bold font-mono text-blue-700 mt-0.5">
                    Rp {{ number_format($report['total_debit'], 0, ',', '.') }}
                </h3>
            </div>
            <div class="w-8 h-8 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 text-xs shrink-0">
                <i class="fa-solid fa-arrow-down-long"></i>
            </div>
        </div>

        <!-- Total Kredit -->
        <div class="bg-white rounded-xl border border-slate-200/90 p-2.5 sm:p-3 shadow-xs flex items-center justify-between">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Kredit</p>
                <h3 class="text-sm sm:text-base font-bold font-mono text-indigo-700 mt-0.5">
                    Rp {{ number_format($report['total_credit'], 0, ',', '.') }}
                </h3>
            </div>
            <div class="w-8 h-8 rounded-lg bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 text-xs shrink-0">
                <i class="fa-solid fa-arrow-up-long"></i>
            </div>
        </div>

        <!-- Status Keseimbangan -->
        <div class="bg-white rounded-xl border border-slate-200/90 p-2.5 sm:p-3 shadow-xs flex items-center justify-between">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Keseimbangan</p>
                <h3 class="text-xs sm:text-sm font-bold mt-0.5 flex items-center gap-1 {{ $isBalanced ? 'text-emerald-600' : 'text-rose-600' }}">
                    <i class="fa-solid {{ $isBalanced ? 'fa-circle-check text-[11px]' : 'fa-circle-xmark text-[11px]' }}"></i>
                    {{ $isBalanced ? 'SEIMBANG' : 'SELISIH' }}
                </h3>
            </div>
            <div class="w-8 h-8 rounded-lg {{ $isBalanced ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-rose-50 border-rose-100 text-rose-600' }} border flex items-center justify-center text-xs shrink-0">
                <i class="fa-solid {{ $isBalanced ? 'fa-scale-balanced' : 'fa-triangle-exclamation' }}"></i>
            </div>
        </div>

        <!-- Jumlah Akun Aktif -->
        <div class="bg-white rounded-xl border border-slate-200/90 p-2.5 sm:p-3 shadow-xs flex items-center justify-between">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Akun Bersaldo</p>
                <h3 class="text-sm sm:text-base font-bold text-slate-800 mt-0.5">
                    {{ count($report['rows']) }} <span class="text-[10px] text-slate-400 font-normal">/ 120 Akun</span>
                </h3>
            </div>
            <div class="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 text-xs shrink-0">
                <i class="fa-solid fa-list-check"></i>
            </div>
        </div>
    </div>

    <!-- Table Card: Pengelompokan Kategori Aset Lancar, Aset Tetap, dll (Kompak & Rapi) -->
    <div class="card-table bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div class="overflow-x-auto">
            <table id="trialBalanceTable" class="w-full text-left text-xs text-slate-800">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase text-[10px] tracking-wider">
                        <th class="py-2.5 px-4 w-32">KODE</th>
                        <th class="py-2.5 px-3">NAMA AKUN</th>
                        <th class="py-2.5 px-4 text-right w-44">SALDO DEBIT</th>
                        <th class="py-2.5 px-4 text-right w-44">SALDO KREDIT</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($groupMeta as $groupKey => $meta)
                        @if(isset($groupedRows[$groupKey]) && count($groupedRows[$groupKey]) > 0)
                            @php
                                $rowsInGroup = $groupedRows[$groupKey];
                                $subtotalDebit = array_sum(array_column($rowsInGroup, 'saldo_debit'));
                                $subtotalCredit = array_sum(array_column($rowsInGroup, 'saldo_credit'));
                            @endphp

                            <!-- CATEGORY SECTION HEADER (Ramping & Proporsional) -->
                            <tr class="section-header-row">
                                <td colspan="4" class="p-0 border-t border-slate-200">
                                    <div class="{{ $meta['bg'] }} px-4 py-1.5 text-white flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <i class="{{ $meta['icon'] }} text-[11px] opacity-90"></i>
                                            <span class="text-xs font-bold tracking-wide uppercase">
                                                {{ $meta['name'] }}
                                            </span>
                                        </div>
                                        <span class="text-[10px] font-mono font-medium bg-black/20 text-white px-2 py-0.5 rounded">
                                            {{ count($rowsInGroup) }} Rekening
                                        </span>
                                    </div>
                                </td>
                            </tr>

                            <!-- ACCOUNT ROWS IN THIS GROUP (Kompak & Rapi) -->
                            @foreach($rowsInGroup as $row)
                                <tr class="hover:bg-blue-50/30 transition">
                                    <td class="py-2 px-4">
                                        <span class="inline-block font-mono font-semibold text-[11px] text-slate-700 bg-slate-100 px-2 py-0.5 rounded border border-slate-200">
                                            {{ $row['code'] }}
                                        </span>
                                    </td>
                                    <td class="py-2 px-3">
                                        <div class="font-medium text-slate-800 text-xs">
                                            {{ $row['name'] }}
                                        </div>
                                    </td>
                                    <td class="py-2 px-4 text-right font-mono font-semibold text-xs text-slate-800">
                                        {{ $row['saldo_debit'] > 0 ? 'Rp ' . number_format($row['saldo_debit'], 0, ',', '.') : '-' }}
                                    </td>
                                    <td class="py-2 px-4 text-right font-mono font-semibold text-xs text-slate-800">
                                        {{ $row['saldo_credit'] > 0 ? 'Rp ' . number_format($row['saldo_credit'], 0, ',', '.') : '-' }}
                                    </td>
                                </tr>
                            @endforeach

                            <!-- SUBTOTAL PER KATEGORI (Ramping) -->
                            <tr class="{{ $meta['subtotal_bg'] }} border-t border-b border-slate-200 font-bold text-xs">
                                <td colspan="2" class="py-1.5 px-4 text-[11px] text-slate-600 uppercase">
                                    Subtotal {{ $meta['name'] }}
                                </td>
                                <td class="py-1.5 px-4 text-right font-mono font-bold text-xs text-slate-800">
                                    {{ $subtotalDebit > 0 ? 'Rp ' . number_format($subtotalDebit, 0, ',', '.') : '-' }}
                                </td>
                                <td class="py-1.5 px-4 text-right font-mono font-bold text-xs text-slate-800">
                                    {{ $subtotalCredit > 0 ? 'Rp ' . number_format($subtotalCredit, 0, ',', '.') : '-' }}
                                </td>
                            </tr>
                        @endif
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-8 text-slate-400 italic text-xs">
                                Tidak ada saldo akun dalam periode yang dipilih.
                            </td>
                        </tr>
                    @endforelse

                    <!-- GRAND TOTAL ROW (Kompak, Seimbang & Double Underline) -->
                    <tr class="bg-slate-900 text-white border-t-2 border-blue-500">
                        <td colspan="2" class="py-2.5 px-4 align-middle">
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-bold uppercase tracking-wider text-white">
                                    TOTAL NERACA SALDO
                                </span>
                                <span class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded {{ $isBalanced ? 'bg-emerald-500/30 text-emerald-300 border border-emerald-400/30' : 'bg-rose-500/30 text-rose-300 border border-rose-400/30' }}">
                                    <i class="fa-solid {{ $isBalanced ? 'fa-check' : 'fa-triangle-exclamation' }} text-[9px]"></i>
                                    {{ $isBalanced ? 'SEIMBANG' : 'SELISIH Rp ' . number_format($balanceDiff, 0, ',', '.') }}
                                </span>
                            </div>
                        </td>
                        <td class="py-2.5 px-4 text-right align-middle font-mono font-bold text-xs sm:text-sm text-white border-b-2 border-double border-white">
                            Rp {{ number_format($report['total_debit'], 0, ',', '.') }}
                        </td>
                        <td class="py-2.5 px-4 text-right align-middle font-mono font-bold text-xs sm:text-sm text-white border-b-2 border-double border-white">
                            Rp {{ number_format($report['total_credit'], 0, ',', '.') }}
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

</div>

<!-- SCRIPT EKSPOR EXCEL NATIVE BERSIH UNTUK NERACA SALDO -->
<script>
function exportTrialBalanceToExcel() {
    var companyName = @json($company->name ?? 'Perusahaan');
    var periodStr = @json(\Carbon\Carbon::parse($startDate)->format('d/m/Y') . ' - ' . \Carbon\Carbon::parse($endDate)->format('d/m/Y'));
    var todayStr = @json(date('Y-m-d'));

    var excelXml = '<' + 'xml><' + 'x:ExcelWorkbook><' + 'x:ExcelWorksheets><' + 'x:ExcelWorksheet>';
    excelXml += '<' + 'x:Name>Neraca Saldo<' + '/x:Name>';
    excelXml += '<' + 'x:WorksheetOptions><' + 'x:DisplayGridlines/><' + '/x:WorksheetOptions><' + '/x:ExcelWorksheet><' + '/x:ExcelWorksheets><' + '/x:ExcelWorkbook><' + '/xml>';

    var html = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">';
    html += '<head><meta charset="utf-8"><!--[if gte mso 9]>' + excelXml + '<![endif]--></head><body>';

    var table = document.getElementById('trialBalanceTable');
    if (!table) {
        alert('Tabel tidak ditemukan.');
        return;
    }

    html += '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse; font-family:Arial, sans-serif; font-size:10pt;">';
    html += '<tr><th colspan="4" style="font-size:13pt; font-weight:bold; text-align:left; background-color:#1e3a8a; color:#ffffff; padding:8px;">' + companyName + ' - LAPORAN NERACA SALDO (TRIAL BALANCE)</th></tr>';
    html += '<tr><th colspan="4" style="text-align:left; background-color:#f1f5f9; color:#475569; padding:6px;">Periode: ' + periodStr + '</th></tr>';
    html += '<tr><td colspan="4" style="height:10px;"></td></tr>';

    var rows = table.querySelectorAll('tr');
    rows.forEach(function(row) {
        var ths = row.querySelectorAll('th');
        var tds = row.querySelectorAll('td');

        if (ths.length > 0) {
            html += '<tr style="background-color:#f8fafc; font-weight:bold; text-align:center;">';
            ths.forEach(function(th) {
                html += '<th style="padding:6px; border:1px solid #cbd5e1;">' + th.innerText.trim() + '</th>';
            });
            html += '</tr>';
        } else if (tds.length > 0) {
            var isSection = row.classList.contains('section-header-row') || tds.length === 1;
            var isTotal = row.innerText.indexOf('TOTAL NERACA SALDO') !== -1;
            var isSubtotal = row.innerText.indexOf('Subtotal') !== -1;

            if (isSection) {
                var sectionTitle = tds[0].innerText.trim().replace(/\s+/g, ' ');
                html += '<tr style="background-color:#2563eb; color:#ffffff; font-weight:bold;">';
                html += '<td colspan="4" style="padding:6px; font-size:10.5pt; color:#ffffff; background-color:#2563eb;">' + sectionTitle + '</td>';
                html += '</tr>';
            } else if (isTotal) {
                var debitVal = tds[1] ? tds[1].innerText.trim().replace(/\s+/g, ' ') : '';
                var creditVal = tds[2] ? tds[2].innerText.trim().replace(/\s+/g, ' ') : '';
                html += '<tr style="background-color:#0f172a; color:#ffffff; font-weight:bold; font-size:10.5pt;">';
                html += '<td colspan="2" style="padding:8px; color:#ffffff; background-color:#0f172a;">TOTAL NERACA SALDO</td>';
                html += '<td style="text-align:right; padding:8px; color:#ffffff; background-color:#0f172a;">' + debitVal + '</td>';
                html += '<td style="text-align:right; padding:8px; color:#ffffff; background-color:#0f172a;">' + creditVal + '</td>';
                html += '</tr>';
            } else if (isSubtotal) {
                var subTitle = tds[0].innerText.trim().replace(/\s+/g, ' ');
                var subDebit = tds[1] ? tds[1].innerText.trim() : '';
                var subCredit = tds[2] ? tds[2].innerText.trim() : '';
                html += '<tr style="background-color:#e2e8f0; font-weight:bold;">';
                html += '<td colspan="2" style="padding:5px; color:#1e293b;">' + subTitle + '</td>';
                html += '<td style="text-align:right; padding:5px; color:#1e293b;">' + subDebit + '</td>';
                html += '<td style="text-align:right; padding:5px; color:#1e293b;">' + subCredit + '</td>';
                html += '</tr>';
            } else {
                var code = tds[0] ? tds[0].innerText.trim() : '';
                var name = tds[1] ? tds[1].innerText.trim().replace(/\s+/g, ' ') : '';
                var debit = tds[2] ? tds[2].innerText.trim() : '';
                var credit = tds[3] ? tds[3].innerText.trim() : '';

                html += '<tr>';
                html += '<td style="padding:5px; border:1px solid #e2e8f0;">' + code + '</td>';
                html += '<td style="padding:5px; border:1px solid #e2e8f0;">' + name + '</td>';
                html += '<td style="text-align:right; padding:5px; border:1px solid #e2e8f0;">' + debit + '</td>';
                html += '<td style="text-align:right; padding:5px; border:1px solid #e2e8f0;">' + credit + '</td>';
                html += '</tr>';
            }
        }
    });

    html += '</table></body></html>';

    var blob = new Blob(['\ufeff', html], { type: 'application/vnd.ms-excel;charset=utf-8' });
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
    a.download = 'Neraca_Saldo_' + todayStr + '.xls';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
}
</script>
@endsection
